...
// `iam`/`hr`/`inventory`/`production`/`sales` fails typecheck at the `t()` call site. `common`/
// `table` reuse @erp/ui's own message types so the app and the package can never drift on those two
// namespaces' key shapes.
