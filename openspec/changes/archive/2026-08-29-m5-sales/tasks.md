...
- [x] 7.3 Boot `pnpm dev` and drive: create customer → quotation (QV) → send → approve →
  ...
  endpoints are gone (covered by `e2e/tests/sales.spec.ts` TC-SALES-01/02 and `e2e/tests/sales-module.spec.ts` TC-SALES-03..12 plus `apps/api/test/integration/sales.int.spec.ts`; the demo `apps/api/src/invoice/` module is confirmed deleted)
