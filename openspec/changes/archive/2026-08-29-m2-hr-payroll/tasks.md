...
- [x] 2.12 `pnpm db:migrate && pnpm db:seed` against dev Postgres; confirm tables + FK + seeds (verified in the 2026-08-16 live-stack session)
...
- [x] 6.2 `pnpm db:generate` clean after migration; `pnpm db:migrate && pnpm db:seed` run
  cleanly against a fresh DB (tables, `user.employeeId` FK, config seeds present) (verified in the 2026-08-16 live-stack session; migrations `0005`/`0006` present in `tooling/drizzle/`)
- [x] 6.3 Boot `pnpm dev` and drive end-to-end: create employee (emp_code `EXT0001`, PII
  ...
  encrypted payslip (covered by `e2e/tests/hr.spec.ts` UAT journey J2 hire-to-payslip, `e2e/tests/hr-module.spec.ts` TC-HR-01..15, and `apps/api/test/integration/hr.int.spec.ts`)
