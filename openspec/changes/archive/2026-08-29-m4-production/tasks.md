...
- [x] 3.1 Add a generic authenticated `@SubscribeMessage("join")` handler to `RealtimeGateway`
  (validate the room, `joinRoom(client, room)`); the JWT handshake already gates the socket
- [x] 3.2 Add a BullMQ **repeatable** monitor job on the `default` queue (cadence configurable,
  default ~60s) + a `BaseWorker` subclass; register the repeatable job at module init (upsert)

## 4. Nest module — `apps/api/src/production`

- [x] 4.1 `RoutingService` — create/list routing templates + steps
- [x] 4.2 `WorkOrderService` — create (issue `wo_no` via SequenceService `WORK_ORDER`;
  **materialize** step snapshot from the template), `GET /{id}` detail, `GET /timeline` feed
  (`is_delayed` computed on read)
- [x] 4.3 `ScanService` — `scan` START/FINISH (append `production_scan`, derive step
...
- [x] 4.4 `ProductionMonitorWorker` — the sweep: subcontracts `SENT` & `sla_due<now` → OVERDUE
...
- [x] 4.5 `SubcontractService` — `subcontract` send (step → OUTSOURCED) and `receive` (→ line)
- [x] 4.6 `CompletionService` — on last-step FINISH, WO → COMPLETED, emit `WorkOrderCompleted`
  via `publishAfterCommit` (correlation_id auto) with an `audit` block
- [x] 4.7 `WipReportService` — `GET /reports/wip` (per department: in-progress + delayed counts)
- [x] 4.8 ts-rest `ProductionController`(s) — in-handler `assertPermissions(user,
...
- [x] 4.9 Verify: `pnpm build && pnpm typecheck && pnpm lint` green; API boots and maps the
  new `/api/v1` production routes

## 5. Tests (spec §4.7)

- [x] 5.1 Scan START on "Sew" ⇒ step IN_PROGRESS + timer running; exceeding `standard_time_min`
  ⇒ `StepDelayed` emitted, supervisor notified, step flagged delayed in the timeline feed
- [x] 5.2 Subcontract a step ⇒ OUTSOURCED + SLA countdown; the monitor flips past-SLA → OVERDUE;
  receive ⇒ step returns to the line / timeline continues
- [x] 5.3 Completing the final step ⇒ WO COMPLETED and exactly one `WorkOrderCompleted`
  (a duplicate delivery does not double-post — M3-side idempotent on `wo_id`)
- [x] 5.4 Editing a routing template after a WO exists ⇒ that WO's materialized steps unchanged
- [x] 5.5 Re-FINISH on a COMPLETED step ⇒ 409; `production_scan` UPDATE/DELETE rejected by the trigger

## 6. Verification

- [x] 6.1 `pnpm build && pnpm typecheck && pnpm lint && pnpm test` green from the repo root
- [x] 6.2 `pnpm db:generate` clean after migration; `pnpm db:migrate && pnpm db:seed` run
  cleanly against a fresh DB (production tables, `production_scan` trigger, `WORK_ORDER` seq) (verified in the 2026-08-16 live-stack session; the `production_scan` append-only trigger ships in `tooling/drizzle/0007_production_scan_append_only.sql`)
- [x] 6.3 Boot `pnpm dev` and drive: create routing template → create WO (steps materialized,
  ...
  and confirm `StepStarted`/`StepFinished`/`StepDelayed` broadcasts (covered by `e2e/tests/production.spec.ts` TC-PROD-01..11 and `apps/api/test/integration/production.int.spec.ts`)
