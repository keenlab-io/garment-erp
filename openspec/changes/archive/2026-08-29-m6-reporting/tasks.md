...
- [x] 6.2 `pnpm db:generate` clean; `pnpm db:migrate` runs cleanly against a DB with M3/M5
  applied (creates `report_schedule` + the three MVs with unique indexes) (verified in the 2026-08-16 live-stack session; `tooling/drizzle/0011_reporting_materialized_views.sql` creates `mv_cogs_monthly`/`mv_sales_daily`/`mv_stock_valuation`)
- [x] 6.3 Boot `pnpm dev` and drive: `GET /reports/{key}` for each group (403 on cost/profit
  ...
  `run-now` (email sent via SMTP) → emit a stock/sales event and confirm the targeted MV refresh (covered by `e2e/tests/reports.spec.ts` TC-RPT-01..11 and `apps/api/test/integration/reporting.int.spec.ts`)
