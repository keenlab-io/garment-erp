...
- [x] 3.10 `pnpm db:migrate && pnpm db:seed` against dev Postgres; confirm tables, the
  `stock_movement` trigger, and seeds (verified in the 2026-08-16 live-stack session; the `stock_movement` append-only trigger ships in `tooling/drizzle/0004_stock_movement_append_only.sql`)
...
- [x] 7.2 `pnpm db:generate` clean after migration; `pnpm db:migrate && pnpm db:seed` run
  cleanly against a fresh DB (tables, trigger, `AA00001` item code, default warehouse/UOMs) (verified in the 2026-08-16 live-stack session)
- [x] 7.3 Boot `pnpm dev` and drive: create item (`AA00001`) → receipt (confirm landed → post)
  → issue → stock count → adjustment → stock-card/valuation reports; confirm a barcode-print
  job returns 202 (covered by `e2e/tests/inventory.spec.ts` UAT journey J3 procure-to-stock, `e2e/tests/inventory-module.spec.ts` TC-INV-01..11, and `apps/api/test/integration/inventory.int.spec.ts`)
